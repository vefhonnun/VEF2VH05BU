...
CSS kvikun (**animation**) er leið til að láta HTML-stök breytast úr einu útliti yfir í annað á ákveðnum tíma. Ólíkt einföldum „transition“ (sem þarf kveikju eins og músarsmell), getur CSS animation keyrt sjálfkrafa og verið mjög flókin.

Hér eru tveir meginhlutar sem þarf til að búa til kvikun:
...
### 2. Animation eiginleikinn (Leikstjórinn)
Þegar þú ert búinn að búa til handritið með `@keyframes`, þarftu að segja ákveðnu staki (t.d. mynd eða hnappi) að nota það. Þar stillir þú:

* **`animation-name`**: Hvaða `@keyframes` á að nota.
* **`animation-duration`**: Hversu margar sekúndur hreyfingin á að taka.
* **`animation-iteration-count`**: Hversu oft á að endurtaka (t.d. `3` sinnum eða `infinite` fyrir endalaust).
* **`animation-timing-function`**: Hraðaferillinn (t.d. `linear` fyrir jafnan hraða eða `ease-in-out`).

---

### Af hverju að nota þetta?
* **Sjálfvirkni:** Þú getur búið til bakgrunna sem hreyfast eða tákn sem athuga athygli án þess að notandinn geri neitt.
